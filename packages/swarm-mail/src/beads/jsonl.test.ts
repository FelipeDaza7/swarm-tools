...
describe("JSONL Export/Import", () => {
  ...
  describe("exportToJSONL", () => {
    ...
    it("exports multiple beads", async () => {
      await adapter.createBead(projectKey, { title: "Task 1", type: "task" });
      await adapter.createBead(projectKey, { title: "Task 2", type: "bug" });
    ...
    it("exports specific beads only", async () => {
      ...
      await adapter.createBead(projectKey, { title: "Task 2", type: "task" });
  ...
  describe("exportDirtyBeads", () => {
    it("exports only dirty beads", async () => {
      ...
      await db.query(
        "INSERT INTO dirty_beads (bead_id, marked_at) VALUES ($1, $2)",
        [bead2.id, Date.now()]
      );
      ...
      expect(result.beadIds).toEqual([bead2.id]);
    });

    it("returns empty for no dirty beads", async () => {
      await adapter.createBead(projectKey, { title: "Task", type: "task" });
  ...
  describe("importFromJSONL", () => {
    it("imports new beads", async () => {
      ...
      const bead = await adapter.getBead(projectKey, "bd-new123");
    ...
    it("updates existing beads", async () => {
      ...
      const updated = await adapter.getBead(projectKey, bead.id);
    ...
    it("dry run does not modify database", async () => {
      ...
      const bead = await adapter.getBead(projectKey, "bd-dry123");
    ...
    it("skipExisting does not update existing beads", async () => {
      ...
      const unchanged = await adapter.getBead(projectKey, bead.id);
