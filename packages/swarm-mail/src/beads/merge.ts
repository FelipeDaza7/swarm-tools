 ...
 */
function makeKey(bead: BeadExport): string {
  // Use JSON for stable key generation
  const key: IssueKey = {
    id: bead.id,
    createdAt: bead.created_at,
    createdBy: undefined, // Not in our BeadExport type, but could be added
  };
 ...
 */
export function isTombstone(bead: BeadExport): boolean {
...
function mergeDependencies(
  left: BeadExport["dependencies"],
  right: BeadExport["dependencies"]
): BeadExport["dependencies"] {
  const seen = new Set<string>();
  const result: BeadExport["dependencies"] = [];
...
function mergeComments(
  left: BeadExport["comments"],
  right: BeadExport["comments"]
): BeadExport["comments"] {
  const seen = new Set<string>();
  const result: BeadExport["comments"] = [];
 ...
 */
function mergeTombstones(left: BeadExport, right: BeadExport): BeadExport {
...
function mergeIssue(
  ...
  left: BeadExport,
  right: BeadExport
): { merged: BeadExport; conflict: string } {
  const merged: BeadExport = {
    id: base.id,
  ...
  merged.status = mergeStatus(base.status, left.status, right.status) as BeadExport["status"];
...
export function merge3Way(
  ...
  const leftMap = new Map<string, BeadExport>();
  for (const bead of left) {
    leftMap.set(makeKey(bead), bead);
  }

  const rightMap = new Map<string, BeadExport>();
...
export function mergeJsonl(
  ...
  const parseJSONL = (jsonl: string): BeadExport[] => {
    if (!jsonl || jsonl.trim() === "") return [];
    return jsonl
      .split("\n")
      .filter((line) => line.trim() !== "")
      .map((line) => JSON.parse(line) as BeadExport);
  };
