...
describe("Dependencies", () => {
  ...
  describe("addDependency", () => {
    test("adds a blocks dependency", async () => {
      const bead1 = await beads.createBead(projectKey, { title: "Task 1", type: "task", priority: 2 });
      const bead2 = await beads.createBead(projectKey, { title: "Task 2", type: "task", priority: 2 });

      const dep = await beads.addDependency(projectKey, bead1.id, bead2.id, "blocks");

      expect(dep.bead_id).toBe(bead1.id);
      expect(dep.depends_on_id).toBe(bead2.id);
      expect(dep.relationship).toBe("blocks");
    });

    test("prevents self-dependency", async () => {
      const bead = await beads.createBead(projectKey, { title: "Task", type: "task", priority: 2 });
    ...
    test("prevents transitive cycle", async () => {
      const bead1 = await beads.createBead(projectKey, { title: "Task 1", type: "task", priority: 2 });
      const bead2 = await beads.createBead(projectKey, { title: "Task 2", type: "task", priority: 2 });
      const bead3 = await beads.createBead(projectKey, { title: "Task 3", type: "task", priority: 2 });
  ...
  describe("getDependents", () => {
    test("returns beads that depend on this bead", async () => {
      ...
      expect(dependents.map(d => d.bead_id).sort()).toEqual([bead2.id, bead3.id].sort());
    });
  });
  ...
  describe("getBlockers", () => {
    test("returns blocker IDs for a bead", async () => {
      const blocker1 = await beads.createBead(projectKey, { title: "Blocker 1", type: "task", priority: 2 });
      const blocker2 = await beads.createBead(projectKey, { title: "Blocker 2", type: "task", priority: 2 });
      const blocked = await beads.createBead(projectKey, { title: "Blocked", type: "task", priority: 2 });
  ...
  describe("rebuildBlockedCache", () => {
    test("rebuilds blocked cache for all beads", async () => {
      const blocker = await beads.createBead(projectKey, { title: "Blocker", type: "task", priority: 2 });
      const blocked1 = await beads.createBead(projectKey, { title: "Blocked 1", type: "task", priority: 2 });
      const blocked2 = await beads.createBead(projectKey, { title: "Blocked 2", type: "task", priority: 2 });
