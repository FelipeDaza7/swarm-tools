...
describe("Beads Projections", () => {
  ...
  describe("bead_created event", () => {
    test("creates bead record", async () => {
      ...
      const bead = await getBead(db, projectKey, "bd-123");
    ...
    test("marks bead as dirty", async () => {
      ...
      const dirtyBeads = await getDirtyBeads(db, projectKey);
      expect(dirtyBeads).toContain("bd-123");
    });
  });

  describe("bead_updated event", () => {
  ...
  describe("bead_status_changed event", () => {
  ...
  describe("query functions", () => {
    test("queryBeads filters by status", async () => {
      await updateProjections(db, {
        type: "bead_created",
        project_key: projectKey,
        bead_id: "bd-123",
      ...
      const openBeads = await queryBeads(db, projectKey, { status: "open" });
      expect(openBeads).toHaveLength(1);
      expect(openBeads[0]?.id).toBe("bd-123");

      const inProgressBeads = await getInProgressBeads(db, projectKey);
      expect(inProgressBeads).toHaveLength(1);
      expect(inProgressBeads[0]?.id).toBe("bd-124");
    });

    test("getNextReadyBead returns unblocked high priority bead", async () => {
      ...
      const ready = await getNextReadyBead(db, projectKey);
      expect(ready?.id).toBe("bd-123"); // Higher priority
    });

    test("getBlockedBeads returns beads with blockers", async () => {
      await updateProjections(db, {
        type: "bead_created",
        project_key: projectKey,
        bead_id: "bd-123",
      ...
      expect(blocked[0]?.bead.id).toBe("bd-124");
  ...
  describe("dirty tracking", () => {
    test("marks bead as dirty", async () => {
      ...
      const dirty = await getDirtyBeads(db, projectKey);
